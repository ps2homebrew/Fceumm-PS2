...
//display list
        if(selection > (n-1)) { selection = selection-n; }
        if(selection < 0) { selection = selection+n; }
        if(selection != oldselect) {
            ...
            browser_primitive("FCEUltra PS2 Beta 0.90","Browser", &BG_TEX, menu_x1, menu_y1, menu_x2, menu_y2);
